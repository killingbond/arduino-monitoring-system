...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="box">
            ...
            <div class="box-body">
            <div class="box-tools">
                <a class="btn btn-primary" href="<?php base_url2()?>size/add">Add Size</a>
                <button class="btn btn-danger" id="btn_delete">Delete</button>
            </div>
            <br>
                <?php
                alert_success_add('Success add size.');
                alert_failed_add('Failed add size!');
                alert_success_update('Success update size.');
                alert_failed_update('Failed update size!');
                alert_success_delete('Success delete size.');
                alert_failed_delete('Failed delete size!');
                ?>
            <div class="box-tools">
            ...
            if($data_table != ''){
                foreach($data_table as $row) {
                    ...
                    <tr>
                        ...
                        <td><?php echo $no ?></td>
                        <td><a href="<?php base_url2()?>size/update/<?php echo $row->user_id ?>"><?php echo $row->username ?></a></td>
                    </tr>
                <?php
                    $no++;
                }
            ...
            <div class="row">
                ...
                <div class="col-sm-7">
                    <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
                        <ul class="pagination">
                            <li <?php if($page==1)echo 'class="disabled"'?>><a <?php if($page>1){?>href="<?php echo base_url()."size/index/1/".$order."/".$max."/".$search;?>"<?php }?>>First</a></li>
                            <li <?php if($page==1)echo 'class="disabled"'?>><a <?php if($page>1){?>href="<?php echo base_url()."size/index/".($page-1)."/".$order."/".$max."/".$search;?>"<?php }?>>Previous</a></li>
                            ...
                            for($i=$start;$i<=$stop;$i++){
                                if($i == $page){
                                    ...
                                    echo '<li><a href="'.base_url()."size/index/".$i."/".$order."/".$max."/".$search.'">'.$i.'</a></li>';
                                }
                            }
                            ?>

                            <li <?php if($page==$max_page)echo 'class="disabled"'?>><a <?php if($page<$max_page){?>href="<?php echo base_url()."size/index/".($page+1)."/".$order."/".$max."/".$search;?>"<?php }?>>Next</a></li>
                            <li <?php if($page==$max_page)echo 'class="disabled"'?>><a <?php if($page<$max_page){?>href="<?php echo base_url()."size/index/".$max_page."/".$order."/".$max."/".$search;?>"<?php }?>>Last</a></li>
                        </ul>
                    </div>
                </div>
            </div>

                <form action="<?php base_url2()?>size/drop" method="post" id="form_delete">
...
<script>
    ...
    $(document).ready(function () {
        ...
        function redir(){
            ...
            window.location.href = '<?php base_url2()?>size/index/' + page + '/' + order + '/' + max + '/' + search;
        }
